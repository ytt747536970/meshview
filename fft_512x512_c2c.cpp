...
void fft512x512_create_plan(CSFFT512x512_Plan* plan, ID3D11Device* pd3dDevice, UINT slices)
{
    ...
    CompileShaderFromFile(L"FX/fft_512x512_c2c.hlsl", "Radix008A_CS", "cs_4_0", &pBlobCS);
    CompileShaderFromFile(L"FX/fft_512x512_c2c.hlsl", "Radix008A_CS2", "cs_4_0", &pBlobCS2);
	assert(pBlobCS);
	assert(pBlobCS2);
...
{
	ReleaseCOM(plan->pSRV_Tmp);
	ReleaseCOM(plan->pUAV_Tmp);
	ReleaseCOM(plan->pBuffer_Tmp);
	ReleaseCOM(plan->pRadix008A_CS);
	ReleaseCOM(plan->pRadix008A_CS2);
	ReleaseCOM(plan->pd3dImmediateContext);

	for (int i = 0; i < 6; i++)
		ReleaseCOM(plan->pRadix008A_CB[i]);
}
