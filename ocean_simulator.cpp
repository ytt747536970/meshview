...
OceanSimulator::OceanSimulator(OceanParameter& params, ID3D11Device* pd3dDevice)
{
    ...
    CompileShaderFromFile(L"FX/ocean_simulator_cs.hlsl", "UpdateSpectrumCS", "cs_4_0", &pBlobUpdateSpectrumCS);
    ...
    ReleaseCOM(pBlobUpdateSpectrumCS);
    ...
    CompileShaderFromFile(L"FX/ocean_simulator_vs_ps.hlsl", "QuadVS", "vs_4_0", &pBlobQuadVS);
    CompileShaderFromFile(L"FX/ocean_simulator_vs_ps.hlsl", "UpdateDisplacementPS", "ps_4_0", &pBlobUpdateDisplacementPS);
    CompileShaderFromFile(L"FX/ocean_simulator_vs_ps.hlsl", "GenGradientFoldingPS", "ps_4_0", &pBlobGenGradientFoldingPS);
	assert(pBlobQuadVS);
	...
	ReleaseCOM(pBlobUpdateDisplacementPS);
	ReleaseCOM(pBlobGenGradientFoldingPS);
...
OceanSimulator::~OceanSimulator()
{
	...
	ReleaseCOM(m_pBuffer_Float2_H0);
	ReleaseCOM(m_pBuffer_Float_Omega);
	ReleaseCOM(m_pBuffer_Float2_Ht);
	ReleaseCOM(m_pBuffer_Float_Dxyz);

	ReleaseCOM(m_pPointSamplerState);

	ReleaseCOM(m_pQuadVB);

	ReleaseCOM(m_pUAV_H0);
	ReleaseCOM(m_pUAV_Omega);
	ReleaseCOM(m_pUAV_Ht);
	ReleaseCOM(m_pUAV_Dxyz);

	ReleaseCOM(m_pSRV_H0);
	ReleaseCOM(m_pSRV_Omega);
	ReleaseCOM(m_pSRV_Ht);
	ReleaseCOM(m_pSRV_Dxyz);

	ReleaseCOM(m_pDisplacementMap);
	ReleaseCOM(m_pDisplacementSRV);
	ReleaseCOM(m_pDisplacementRTV);

	ReleaseCOM(m_pGradientMap);
	ReleaseCOM(m_pGradientSRV);
	ReleaseCOM(m_pGradientRTV);

	ReleaseCOM(m_pUpdateSpectrumCS);
	ReleaseCOM(m_pQuadVS);
	ReleaseCOM(m_pUpdateDisplacementPS);
	ReleaseCOM(m_pGenGradientFoldingPS);

	ReleaseCOM(m_pQuadLayout);

	ReleaseCOM(m_pImmutableCB);
	ReleaseCOM(m_pPerFrameCB);

	ReleaseCOM(m_pd3dImmediateContext);

#ifdef CS_DEBUG_BUFFER
	ReleaseCOM(m_pDebugBuffer);
#endif
}
